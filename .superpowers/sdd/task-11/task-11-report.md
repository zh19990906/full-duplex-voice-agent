...
Task 11 now covers the original pause/resume/backchannel behavior plus the review-driven fixes:

- `ResponseCheckpointStore` keeps four monotonic cursors and now appends per-segment audio chunks in order instead of overwriting them.
- `PlaybackCoordinator` tracks monotonic `playback_attempt_id` values once a response is resumed, returns the active attempt on `ResumePlan`, and rejects stale or missing ACK attempts when attempt-aware playback is active.
