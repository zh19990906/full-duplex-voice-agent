...
if [[ "${OFFLINE_MODE:-0}" != "1" ]]; then
  if ! command -v hf >/dev/null 2>&1 && ! command -v huggingface-cli >/dev/null 2>&1; then
    echo "error: hf or huggingface-cli is required; use OFFLINE_MODE=1 for directory preparation only" >&2
    exit 1
  fi
else
  echo "offline mode: skipping HuggingFace CLI check"
fi

cd "${REPOSITORY_ROOT}"
args=(--config "${CONFIG_FILE}" --provider huggingface --prepare)
