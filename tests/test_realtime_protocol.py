...
class RealtimeProtocolTests(unittest.TestCase):
    ...
    def test_decode_rejects_invalid_header_or_truncated_frame(self):
        valid = encode_audio_frame(AudioFrameHeader(1, 0.0), b"\x00\x00")
        unsupported_version = bytes([2]) + valid[1:]
        non_16khz = struct.pack("!BIdIB", 1, 1, 0.0, 8000, 1) + b"\x00\x00"
        stereo = struct.pack("!BIdIB", 1, 1, 0.0, 16000, 2) + b"\x00\x00"

        for frame in (unsupported_version, non_16khz, stereo, valid[:-1], b"\x01"):
